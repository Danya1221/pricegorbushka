...
class AccountManager:
    ...
    async def attach_listener(self, slot: int):
        ...
        # Resolve once so @username / numeric ids work consistently.
        try:
        ...
        # Supplier 2 is a passive price source. Import existing recent history
        # immediately so the user does not have to wait for a new post.
        if slot == 2:
            try:
                messages = await client.get_messages(entity, limit=100)
                # Parse individual messages first.
                total = 0
                for msg in reversed(messages):
                    total += await process_text(getattr(msg, "raw_text", "") or "")

                # Some suppliers split one price over several consecutive messages.
                # Try the recent history as one chronological text block as well.
                combined = "\n".join(
                ...
                if combined:
                    ...
                    if parsed:
                        await upsert_parsed(source_name, parsed)
                        total = max(total, len(parsed))
                        await maybe_sync_catalog()

                log.info("%s initial history import complete, parsed=%d", source_name, total)
            except Exception:
