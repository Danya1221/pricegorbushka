...
async def bind_here(m: Message):
    if not admin_only(m): return
    if m.chat.type == "private":
        await m.answer("Эту команду нужно отправить именно в группе, куда публикуем прайс.")
        return
    await set_setting("target_chat_id", str(m.chat.id))
    await set_setting("target_chat_title", m.chat.title or str(m.chat.id))
    # Old slots may belong to a different group; never try to edit them here.
    await set_catalog_slots([])
    await m.answer("✅ Эта группа привязана как группа продажного прайса.")
...
async def target_menu(c: CallbackQuery):
    ...
    await c.message.edit_text(
        ...
        "Чтобы привязать/сменить группу:\n"
        "1. Добавь этого бота в нужную группу.\n"
        "2. Напиши в той группе <code>/bindhere</code>.\n\n"
