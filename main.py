...
def samsung_display_variant(p: Product, heading: str) -> str:
    """Remove repeated Samsung/model prefix from an item under its heading."""
    n = clean_spaces(strip_flags(p.name))
    ...
    return n.strip(" -—–") or clean_spaces(strip_flags(p.name))


async def render_catalog() -> list[tuple[str, str]]:
    rows = await effective_products()

    # Unknown items are kept in DB but not published until classified.
    ...
    for cat in sorted(by_cat, key=lambda c: order.get(c, 9999)):
        if cat == "smartphones" and set(by_cat[cat].keys()) == {"Samsung"}:
            category_heading = "📱 Samsung"
        else:
            category_heading = f"{CATEGORY_EMOJI.get(cat,'📦')} {CATEGORY_TITLE.get(cat, cat)}"

        parts = [
            f"<b>{html.escape(category_heading)}</b>",
            ""
        ]

        for brand in sorted(by_cat[cat]):
            brand_items = by_cat[cat][brand]

            # Samsung smartphones: series heading first, then model groups.
            # Example:
            # Samsung Galaxy A
            # A27
            #   • ...
            ...
            # A57
            #   • ...
            if cat == "smartphones" and brand == "Samsung":
                series_groups: dict[str, dict[str, list[tuple[Product, int]]]] = {}
                fallback_items: list[tuple[Product, int]] = []

                for p, price in brand_items:
                    heading = samsung_display_model(p.name)
                    if not heading:
                        fallback_items.append((p, price))
                        continue

                    if re.search(r"\bGalaxy\s+A\d+", p.name, re.I):
                        series = "Samsung Galaxy A"
                        model_match = re.search(r"\bGalaxy\s+(A\d+)\b", p.name, re.I)
                        model = model_match.group(1).upper() if model_match else heading
                    elif re.search(r"\bGalaxy\s+S\d+", p.name, re.I):
                        ...
                        if model_match:
                            ...
                            model = heading.replace("Samsung Galaxy ", "")
                    elif re.search(r"\bGalaxy\s+Z\s*(?:Flip|Fold)", p.name, re.I):
                    ...
                    series_groups.setdefault(series, {}).setdefault(model, []).append((p, price))

                series_order = ["Samsung Galaxy A", "Samsung Galaxy S", "Samsung Galaxy Fold"]
                for series in series_order:
                    models = series_groups.get(series)
                    if not models:
                        continue

                    parts.append(f"<b>{html.escape(series)}</b>")
                    parts.append("")

                    for model in sorted(models, key=lambda x: x.lower()):
                        parts.append(f"<b>{html.escape(model)}</b>")
                        ...
                        for p, price in sorted(
                            ...
                            parts.append(
                                f"• {html.escape(variant)} — <b>{price:,}</b>".replace(",", " ")
                            )
                        parts.append("")

                if fallback_items:
                    parts.append("<b>Samsung</b>")
                    for p, price in sorted(
                        fallback_items,
                        key=lambda x: display_name(x[0]).lower()
                    ):
                        parts.append(
                            f"• {html.escape(display_name(p))} — <b>{price:,}</b>".replace(",", " ")
                        )
                    parts.append("")
                continue
    ...
    for cat, body in blocks:
        ...
        if msg_id:
            try:
                ...
                # Do NOT create a duplicate just because edit failed.
                # Keep the same mapping and report/log the problem.
                log.error(
