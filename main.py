...
def detect_category(name: str) -> str:
    ...
    if any(x in low for x in ["wayfarer", "ray-ban meta"]):
        return "glasses"
    if any(x in low for x in ["watch", "fit3", "fitbit", "garmin", "forerunner", "vivoactive", "coros pace", "galaxy ring"]):
        return "watches"
...
async def poll_slot(c: CallbackQuery):
    ...
    command = (await get_setting(f"supplier{slot}_command", "/prices" if slot == 1 else "")).strip() or "не задана"
...
def model_group_label(name: str) -> str:
    """Return a customer-friendly base model used for bulk selection."""
    n = clean_spaces(strip_flags(name).replace("**", "").replace("\u00a0", " "))
    low = n.lower()

    # LEGO: treat the entire brand as one selectable group.
    if re.search(r"\bLEGO\b", n, re.I):
        return "LEGO"

    # Miele: group by the base model family, e.g. L1 / HX1 / CX1.
    # Works even if supplier puts extra codes/text before "Miele".
    miele_pos = re.search(r"\bMiele\b", n, re.I)
    if miele_pos:
        after_miele = n[miele_pos.end():].strip()
        m = re.search(r"\b([A-Z]{1,4}\d+[A-Z]?)\b", after_miele, re.I)
        if m:
            return f"Miele {m.group(1).upper()}"
        return "Miele"

    # Dyson: HS08 / HS09 / Airstrait etc.
    m = re.search(r"\bDyson\s+(HS\d+[A-Z]?|HD\d+[A-Z]?|HT\d+[A-Z]?|HP\d+[A-Z]?|TP\d+[A-Z]?|BP\d+[A-Z]?|V\d+[A-Z]?|Gen5detect|Airstrait|Airwrap)\b", n, re.I)
    if m:
        return clean_spaces(m.group(0))

    # Ray-Ban Meta: group all colors/sizes of the same frame + generation.
    m = re.search(r"\bRay[\s-]*Ban\s+Meta\s+(Skyler|Wayfarer|Headliner)(?:\s*\((Gen\s*\d+)\))?", n, re.I)
    if m:
        frame = m.group(1).title()
        gen = clean_spaces(m.group(2) or "")
        return f"Ray-Ban Meta {frame}" + (f" ({gen})" if gen else "")

    # Google Fitbit Air.
    if "fitbit air" in low:
        return "Google Fitbit Air"

    # Harman Kardon families.
    m = re.search(r"\bHarman\s+Kardon\s+(Aura\s+Studio\s+\d+|Onyx\s+Studio\s+\d+|Soundsticks\s+\d+)\b", n, re.I)
    if m:
        return clean_spaces(m.group(0))

    # Insta360 model families.
    m = re.search(r"\b(?:Insta\s*360|Insta360)\s+(GO\s+Ultra(?:\s+Creator\s+Bundle)?|Go3S(?:\s+Standard)?|X\d+)\b", n, re.I)
    if m:
        return "Insta360 " + clean_spaces(m.group(1))

    # Samsung Galaxy common families.
    m = re.search(
        r"\bGalaxy\s+("
        r"S\d+(?:\s+(?:Ultra|Plus|\+|FE))?|"
        r"Z\s*(?:Flip|Fold)\s*\d+(?:\s+Ultra)?|"
        r"Tab\s+[A-Z]\d+(?:\s+(?:Lite|FE|\+|Pro))?|"
        r"Buds\s+\d+(?:\s+Pro)?|"
        r"Watch(?:\s+Ultra|\d+)?"
        r")\b",
        n, re.I
    ...
    if m:
        return "Samsung Galaxy " + clean_spaces(m.group(1))

    # Xiaomi / Redmi / POCO / Realme basic model family.
    m = re.search(r"\b(Xiaomi|Redmi|POCO|Realme)\s+([A-Za-z0-9]+(?:\s+(?:Pro\+?|Ultra|Plus|T|FE|5G))?)\b", n, re.I)
    if m:
        return clean_spaces(f"{m.group(1)} {m.group(2)}")

    # Generic fallback: brand + a compact model stem before variant details.
    brand = detect_brand(n)
    tail = n
    if tail.lower().startswith(brand.lower()):
        tail = tail[len(brand):].strip(" -")
    # Stop before obvious variant details in parentheses, slash-colors, memory/color details.
    tail = re.split(r"\s+\(|\s+-\s+|,", tail, maxsplit=1)[0]
    words = tail.split()
    stem = " ".join(words[:3]) if words else ""
...
async def show_source_products(message: Message, source: str, page: int = 0):
    ...
    selected_models = sum(1 for _, g in grouped if g["items"] and all(p.enabled for p in g["items"]))

    pages = max(1, (total_models + PRODUCTS_PER_PAGE - 1) // PRODUCTS_PER_PAGE)
    page = max(0, min(page, pages - 1))
    start = page * PRODUCTS_PER_PAGE
    chunk = grouped[start:start + PRODUCTS_PER_PAGE]
    ...
    for token, g in chunk:
        ...
        if g["label"] == "LEGO":
            ...
            label = f"{mark} {g['label'][:42]} · {len(items)} вар."
    ...
    body = (
        ...
        f"Выбрано моделей: <b>{selected_models}</b> из <b>{total_models}</b>.\n"
        "Нажми на модель один раз — включатся <b>все её варианты</b> "
        "(цвета, вилки, комплектации и т.д.).\n\n"
        "✅ — вся модель включена, 🟡 — включена частично, ▫️ — выключена."
    )
    try:
