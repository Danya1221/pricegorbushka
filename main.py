...
async def should_catalog_be_open() -> tuple[bool, str]:
    ...
    # Hard daily close from 19:00 until a supplier opens again on a later day.
    # This prevents stale "open" state from reopening the catalog at midnight.
    ...
    if closed_at:
        # After any previous automatic close, require a fresh open signal
        # from at least one supplier before restoring the catalog.
        if not await any_supplier_opened_after(closed_at):
            return False, "ждём открытия поставщика"

    return True, "есть открытый поставщик"
