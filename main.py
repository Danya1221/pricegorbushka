...
def normalize_database_url(url: str) -> str:
    """Force every PostgreSQL URL to use SQLAlchemy's asyncpg dialect."""
    url = (url or "").strip()
    postgres_prefixes = (
        "postgres://",
        "postgresql://",
...
async def polls_menu(c: CallbackQuery):
    ...
    for slot in (1, 2):
        ...
        kb.button(text=f"{state} Поставщик {slot} · {interval} мин · {cmd[:18]}", callback_data=f"poll:{slot}")
    kb.button(text="⬅️ Меню", callback_data="menu")
    kb.adjust(1)
    await c.message.edit_text(
        "<b>Автозапрос прайсов</b>\n\nКаждый аккаунт сам отправляет команду своему боту-поставщику по заданному интервалу.",
        parse_mode="HTML", reply_markup=kb.as_markup())
    await c.answer()
...
async def init_db():
    ...
    for slot in (1, 2):
        await set_setting(f"supplier{slot}_interval", await get_setting(f"supplier{slot}_interval", "30"))
        await set_setting(f"supplier{slot}_command", await get_setting(f"supplier{slot}_command", ""))
        await set_setting(f"supplier{slot}_poll_enabled", await get_setting(f"supplier{slot}_poll_enabled", "0"))

async def main():
    await init_db()
    await accounts.start_existing()
    poll_tasks = [asyncio.create_task(accounts.polling_loop(slot)) for slot in (1, 2)]
    try:
