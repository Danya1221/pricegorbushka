...
def detect_brand(name: str) -> str:
    low = clean_spaces(strip_flags(name)).lower()

    if re.search(r"\bapple\b|\biphone\b|\bairpods\b|\bmacbook\b|\bipad\b|\bapple watch\b", low):
        return "Apple"
    if re.search(r"\bsamsung\b|\bgalaxy\b", low):
        return "Samsung"
    if re.search(r"\bdyson\b", low):
        return "Dyson"
    if re.search(r"\bcanon\b|\bpowershot\b", low):
        return "Canon"
    if re.search(r"\bkodak\b", low):
        return "Kodak"
    if re.search(r"\bplaud\b", low):
        return "Plaud"
    if re.search(r"\bmiele\b", low):
        return "Miele"
    if re.search(r"\bdji\b|\bosmo\b", low):
        return "DJI"
    if re.search(r"\blego\b", low):
        return "LEGO"
    if re.search(r"\bbowers\s*(?:&|and)\s*wilkins\b|\bb&w\b|\bpx7\b|\bpx8\b", low):
        return "B&W Headphones"
    if "harman kardon" in low:
        return "Harman Kardon"
    if "ray-ban" in low or "ray ban" in low or any(x in low for x in ("wayfarer", "skyler", "headliner")):
        return "Ray-Ban Meta"
    if "fujifilm" in low or "instax" in low:
        return "Fujifilm"
    if "insta 360" in low or "insta360" in low:
        return "Insta360"
    if "redmi" in low:
        return "Redmi"
    if "poco" in low:
...
def detect_category(name: str) -> str:
    ...
    # Large ecosystems get their own catalog messages.
    if re.search(r"\bgalaxy\s+(?:a\d+|s\d+|z\s*(?:flip|fold)\s*\d+)", low):
        return "samsung"
    ...
    # All Dyson products live in one Dyson block; model formatting handles HS/HD/V/etc.
    if re.search(r"\bdyson\b", low):
        return "dyson"
    ...
    # One Cameras block: photo cameras + action cameras + stabilizers.
    if any(x in low for x in [
        "canon", "powershot", "eos", "ixus",
        "kodak", "fujifilm", "instax",
        "insta360", "insta 360", "gopro",
        "osmo action", "osmo mobile", "dji action", "dji om ",
        "action camera", "экшн", "gimbal", "стабилизатор",
        "camera", "фотоаппарат"
    ]):
        return "cameras"
    ...
    if any(x in low for x in [
        "buds", "quietcomfort", "voyager free", "headphone", "earbuds",
        "науш", "px7", "px8", "bowers & wilkins", "b&w"
    ]):
        return "headphones"

    if any(x in low for x in ["wayfarer", "skyler", "headliner", "ray-ban meta", "ray ban meta"]):
        return "glasses"
    ...
    if any(x in low for x in ["notebook", "ноутбук"]):
        return "laptops"

    if any(x in low for x in ["tab ", "pad ", "tablet", "планшет"]):
        return "tablets"

    if any(x in low for x in ["power bank", "powerbank", "пауэрбанк"]):
        return "powerbanks"
    ...
    if any(x in low for x in ["smarttag", "smart tag", "умный дом"]):
        return "smart_home"
    ...
    # Non-Samsung / non-Apple phones.
    if any(x in low for x in [
        "redmi ", "note ", "xiaomi ", "realme ", "vivo ",
        "poco ", "pixel "
    ]):
        return "smartphones"

    if any(x in low for x in [
        "case", "cover", "charger", "adapter", "cable",
        "кабель", "чехол", "заряд"
    ]):
        return "accessories"

    # Unknown items are NEVER sent to a generic "Other" catalog message.
    return "unclassified"

def canonicalize(name: str) -> str:
    """
    Aggressive but variant-safe cross-supplier dedupe identity.

    Normalizes harmless supplier differences while preserving:
      model, generation, RAM/storage, color, size, edition/bundle.
    """
    s = strip_flags(name).lower()
    s = s.replace("–", "-").replace("—", "-").replace("ё", "е")
    s = clean_spaces(s)

    # Equivalent spellings / punctuation.
    substitutions = [
        (r"\bgrey\b", "gray"),
        (r"\bspace grey\b", "space gray"),
        (r"\bcolour\b", "color"),
        (r"\bgen\s+(\d+)\b", r"gen\1"),
        (r"\bgeneration\s+(\d+)\b", r"gen\1"),
        (r"\bwi[\s-]*fi\b", "wifi"),
        (r"\be[\s-]*sim\b", "esim"),
        (r"\bpro\s+max\b", "pro max"),
        (r"\bultra\s+wide\b", "ultrawide"),
    ]
    for pattern, repl in substitutions:
        s = re.sub(pattern, repl, s, flags=re.I)

    # Samsung fold spacing: Z Fold 8 == Z Fold8; Z Flip 8 == Z Flip8.
    s = re.sub(r"\bz\s*(fold|flip)\s+(\d+)\b", r"z \1\2", s, flags=re.I)

    # RAM/storage notation:
    # 12/512GB == 12 / 512 Gb
    s = re.sub(
        r"(?<!\d)(\d{1,3})\s*/\s*(\d{2,4})(?:\s*(?:gb|гб))?",
        r" \1 \2 ",
        s,
        flags=re.I,
    )

    # Standalone storage suffix.
    s = re.sub(r"(?<!\d)(\d{2,4})\s*(?:gb|гб)\b", r" \1 ", s, flags=re.I)
    s = re.sub(r"\b(\d)\s*(?:tb|тб)\b", r"\1tb", s, flags=re.I)

    # Human size notation consistency.
    s = re.sub(r"\bsize\s*s?(\d{2})\b", r"s\1", s, flags=re.I)
    s = re.sub(r"\bразмер\s*s?(\d{2})\b", r"s\1", s, flags=re.I)

    # Technical supplier identifiers that do not define the retail variant.
    tech_patterns = [
        r"(?<![a-z0-9])sm-[a-z0-9]+(?:/[a-z0-9]+)*(?![a-z0-9])",
        r"\bupc\s*:?\s*\d+\b",
        r"\brw\d{4}[a-z]?\b",
        r"\bp/n\s*:?\s*[a-z0-9-]+\b",
        r"\b010-[0-9-]+\b",
    ...
    # Supplier stock/article prefix at beginning, e.g. 601/7I50.
    s = re.sub(r"^\s*[a-z0-9]{3,}(?:/[a-z0-9]{2,})+\s+", "", s, flags=re.I)

    # Remove harmless bracket wrappers but keep their content.
    s = s.replace("(", " ").replace(")", " ").replace("[", " ").replace("]", " ")

    # Brand aliases / duplicate brand spellings.
    s = re.sub(r"\bray[\s-]*ban\b", "rayban", s, flags=re.I)
    s = re.sub(r"\bin\s*sta\s*360\b", "insta360", s, flags=re.I)
    s = re.sub(r"\bharman\s+kardon\b", "harmankardon", s, flags=re.I)
    s = re.sub(r"\bbowers\s*(?:&|and)\s*wilkins\b", "bw", s, flags=re.I)
    ...
    # Remove adjacent duplicate tokens introduced by supplier formatting,
    # e.g. "Ray Ban Meta Ray Ban Skyler".
    compact = []
    for token in tokens:
        if compact and compact[-1] == token:
            continue
        compact.append(token)

    # Ignore only non-variant noise words.
    noise = {
        "slim", "original", "ориг", "оригинал",
        "шт", "pcs", "pc",
    }
    compact = [t for t in compact if t not in noise]
...
async def effective_products():
    ...
    # Defensive second-pass dedupe for old DB rows with stale canonical values.
    deduped: dict[str, tuple[Product, int]] = {}
    for p, final_price in result:
        key = canonicalize(p.name)
        current = deduped.get(key)

        if current is None:
            deduped[key] = (p, final_price)
            continue

        # Own stock always has priority over supplier duplicates.
        current_p, current_price = current
        if p.source == "own" and current_p.source != "own":
            deduped[key] = (p, final_price)
            continue
        if current_p.source == "own" and p.source != "own":
            continue

        # Otherwise keep the cheaper final price.
...
def is_rayban_meta(name: str) -> bool:
    n = clean_spaces(strip_flags(name)).lower()
    return (
        ("ray" in n and "ban" in n and "meta" in n)
        or any(x in n for x in ("wayfarer", "skyler", "headliner"))
    )


def rayban_meta_model(name: str) -> str | None:
    n = clean_spaces(strip_flags(name))
    m = re.search(r"\b(Wayfarer|Skyler|Headliner)\b", n, re.I)
    return m.group(1).title() if m else None


def rayban_meta_variant(name: str, model: str) -> str:
    n = clean_spaces(strip_flags(name))

    # Remove commercial/technical prefixes and repeated branding.
    n = re.sub(r"^\s*📌?\s*[A-Z0-9]+(?:/[A-Z0-9]+)*\s+", "", n, flags=re.I)
    n = re.sub(r"\bRay[\s-]*Ban\s+Meta\b", " ", n, flags=re.I)
    n = re.sub(r"\bMeta\s+Ray[\s-]*Ban\b", " ", n, flags=re.I)

    # Remove UPC and frame SKU/model codes (RW4010/RW4012/RW4013/RW4014 etc.).
    n = re.sub(r"\bUPC\s*:?\s*\d+\b", " ", n, flags=re.I)
    n = re.sub(r"\bRW\d{4}[A-Z]?\b", " ", n, flags=re.I)

    # Remove leading stock/article codes that can contain slash.
    n = re.sub(r"^\s*[A-Z0-9]{3,}(?:/[A-Z0-9]{2,})+\s+", "", n, flags=re.I)

    # Remove repeated model names, then put one clean model in renderer.
    n = re.sub(r"\b" + re.escape(model) + r"\b", " ", n, flags=re.I)

    # Normalize generation.
    n = re.sub(r"\(\s*Gen\s*(\d+)\s*\)", r"Gen\1", n, flags=re.I)
    n = re.sub(r"\bGen\s+(\d+)\b", r"Gen\1", n, flags=re.I)

    # Normalize size spellings but KEEP the size because each size is a separate variant.
    # "Размер S52 (M)" / "S52 (M)" / "Size 52" -> "S52 (M)" / "S52"
    n = re.sub(r"\b(?:Размер|Size)\s*S?(\d{2})\s*(\([MLS]\))?", r"S\1 \2", n, flags=re.I)
    n = re.sub(r"\bS\s*(\d{2})\b", r"S\1", n, flags=re.I)

    # Human-readable Ray-Ban frame sizes.
    # Supplier size codes: S50 = M, S52 = M, S53 = L.
    # Keep the numeric frame size too so different sizes never merge.
    size_labels = {"50": "M", "53": "L"}
    def _size_label(match):
        num = match.group(1)
        label = size_labels.get(num)
        return f"S{num} ({label})" if label else f"S{num}"

    n = re.sub(r"\bS(50|53)\b(?:\s*\([MLS]\))?", _size_label, n, flags=re.I)

    # Clean punctuation/spacing left after removed codes.
    n = re.sub(r"\s*/\s*", " / ", n)
    n = re.sub(r"\(\s+", "(", n)
    n = re.sub(r"\s+\)", ")", n)
    n = clean_spaces(n).strip(" -—–/")

    return n


def rayban_variant_sort_key(name: str, model: str):
    v = rayban_meta_variant(name, model)
    # Customer order: M first, then L.
    if re.search(r"\bS50\s*\(M\)", v, re.I):
        size_order = 0
    elif re.search(r"\bS53\s*\(L\)", v, re.I):
        size_order = 1
    else:
        size_order = 99
    return (size_order, v.lower())
...
def render_brand_with_model_groups(parts: list[str], brand: str, items: list[tuple[Product, int]]):
    """
    Universal renderer, with dedicated parsers for product families that have
    technical supplier codes in their names.
    """

    # Ray-Ban Meta must be grouped by FRAME FAMILY, never by SKU/UPC/RW code.
    rayban_items = [(p, price) for p, price in items if is_rayban_meta(p.name)]
    non_rayban_items = [(p, price) for p, price in items if not is_rayban_meta(p.name)]

    if rayban_items:
        parts.append("<b>Ray-Ban Meta</b>")
        rb_groups: dict[str, list[tuple[Product, int]]] = {}
        fallback: list[tuple[Product, int]] = []

        for p, price in rayban_items:
            model = rayban_meta_model(p.name)
            if model:
                rb_groups.setdefault(model, []).append((p, price))
            else:
                fallback.append((p, price))

        rb_order = {"Wayfarer": 0, "Skyler": 1, "Headliner": 2}
        for model in sorted(rb_groups, key=lambda m: (rb_order.get(m, 99), m.lower())):
            for p, price in sorted(
                rb_groups[model],
                key=lambda x: rayban_variant_sort_key(x[0].name, model)
            ):
                variant = rayban_meta_variant(p.name, model)
                parts.append(
        ...
        # Unknown Ray-Ban lines stay visible, but never become fake model groups.
        for p, price in fallback:
            clean = rayban_meta_variant(p.name, "")
            parts.append(f"• {html.escape(clean)} — <b>{price:,}</b>".replace(",", " "))
        if fallback:
            parts.append("")

        if not non_rayban_items:
            return

    # Render the rest of this brand normally.
    if rayban_items:
        items = non_rayban_items
...
async def render_catalog() -> list[tuple[str, str]]:
    ...
    # Reclassify at render time too: old DB rows may still contain stale categories
    # from previous builds. This prevents Dyson/Canon/Plaud from staying in "other"
    # until the supplier sends a new price.
    for p, _price in rows:
        p.category = detect_category(p.name)
        p.brand = detect_brand(p.name)
    # Never publish unknown/"other" products. They stay in DB/admin selection
    # until the classifier knows their real category.
    rows = [
        (p, price) for p, price in rows
        if p.category not in {"unclassified", "other", "другое", "Other"}
    ]

    # Final catalog-level duplicate protection after reclassification.
    final_rows: dict[str, tuple[Product, int]] = {}
    for p, price in rows:
        key = f"{p.category}|{canonicalize(p.name)}"
        current = final_rows.get(key)
        if current is None or price < current[1]:
            final_rows[key] = (p, price)
    ...
    for cat in sorted(by_cat, key=lambda c: order.get(c, 9999)):
        ...
        if cat == "samsung" and samsung_items and not other_items:
...
async def target_menu(c: CallbackQuery):
    ...
    # Answer immediately so Telegram never keeps the button spinning
    # while we read DB/settings.
    try:
        await c.answer()
    except Exception:
        pass
    ...
    try:
        ...
        message_map = await get_catalog_message_map()
        managed_count = len(message_map)
        ...
        body = (
            ...
            f"Статус: {status}\n"
            f"Управляемых товарных сообщений: <b>{managed_count}</b>\n\n"
            "Для привязки или смены просто перешли сообщение из нужной группы/канала.\n"
            "Гарантия / выдача ботом не редактируется."
        )
        ...
        log.exception("target menu failed")
        try:
            await c.message.answer(
                "❌ Не удалось открыть меню группы прайса.\n"
...
@dp.message(lambda m: USER_STATE.get(getattr(getattr(m, "from_user", None), "id", 0), {}).get("mode") == "bind_target_forward")
async def target_bind_forwarded(m: Message):
    ...
    # Aiogram 3 represents forwarded origins differently depending on whether
    # the source is a group, supergroup or channel. Read every supported form.
    source_chat = None
    ...
    if origin is not None:
        ...
        if source_chat is None:
            source_chat = getattr(origin, "sender_chat", None)

    if source_chat is None:
        source_chat = getattr(m, "forward_from_chat", None)

    # Some forwarded channel posts expose sender_chat directly.
    if source_chat is None:
        source_chat = getattr(m, "sender_chat", None)

    if source_chat is None:
        await m.answer(
            "❌ Не смог определить источник пересланного сообщения.\n"
            "Перешли обычное сообщение именно из нужной группы/канала."
        )
        return

    chat_type = str(getattr(source_chat, "type", "")).lower()
    if not any(x in chat_type for x in ("group", "supergroup", "channel")):
        await m.answer("❌ Это сообщение переслано не из группы или канала.")
        return
    ...
    try:
        ...
        log.warning("Target chat detected but bot cannot access it: %s / %s", chat_id, e)
        await m.answer(
            "❌ Канал/группу вижу, но управляющий бот не имеет к нему доступа.\n"
            "Добавь бота туда администратором и перешли сообщение ещё раз."
        )
        return
    ...
    # Telegram message IDs from the previous target must never be reused.
    ...
    await m.answer(
        f"✅ Новая группа/канал сохранён: <b>{html.escape(title)}</b>\n\n"
        "Старые ID товарных сообщений сброшены. "
        "Следующее обновление создаст каталог уже здесь.",
        parse_mode="HTML",
        reply_markup=kb.as_markup(),
    )
